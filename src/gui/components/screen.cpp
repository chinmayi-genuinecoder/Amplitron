...
extern "C" {
EMSCRIPTEN_KEEPALIVE void load_ir_callback_screen(uintptr_t cab_ptr, const char* path) {
    if (cab_ptr && path) {
...
void ScreenComponent::render(ImDrawList* dl, ImVec2 p0, float pedal_width, float zoom, const ScreenProps& props) {
    ...
    switch (props.type) {
        case ScreenType::Tuner:
            render_tuner_display(dl, p0, pedal_width, zoom, props);
            break;
        case ScreenType::Cabinet:
            render_ir_cabinet_display(dl, p0, pedal_width, zoom, props);
            break;
        case ScreenType::Looper:
            render_looper_display(dl, p0, pedal_width, zoom, props);
            break;
        case ScreenType::MultiBandCompressor:
            render_multiband_compressor_display(dl, p0, pedal_width, zoom, props);
            break;
    }
}
...
void ScreenComponent::render_tuner_display(ImDrawList* dl, ImVec2 p0, float pedal_width, float zoom, const ScreenProps& props) {
    ...
    if (tuner) {
        ...
        display_y += 8 * zoom;
        bool mute_on = props.effect->params()[0].value >= 0.5f;
        ...
        if (ImGui::IsItemClicked()) {
            ...
            if (props.engine) {
                props.engine->push_param_change(props.index, 0, new_val);
            }
        }
...
void ScreenComponent::render_ir_cabinet_display(ImDrawList* dl, ImVec2 p0, float pedal_width, float zoom, const ScreenProps& props) {
    ...
    if (ir_cab) {
        ...
        if (ir_cab->has_ir()) {
            ...
            char clear_id[64];
            snprintf(clear_id, sizeof(clear_id), "Clear##ir_clear_%d", props.index);
...
void ScreenComponent::render_looper_display(ImDrawList* dl, ImVec2 p0, float pedal_width, float zoom, const ScreenProps& props) {
    ...
    char rec_id[64];
    std::snprintf(rec_id, sizeof(rec_id), "Record##looper_rec_%d", props.index);
    ...
    char play_id[64];
    std::snprintf(play_id, sizeof(play_id), "Play/Stop##looper_play_%d", props.index);
    ...
    char dub_id[64];
    std::snprintf(dub_id, sizeof(dub_id), "Overdub##looper_dub_%d", props.index);
    ...
    ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(0.50f, 0.20f, 0.15f, 1.0f));
    char clr_id[64];
    std::snprintf(clr_id, sizeof(clr_id), "Clear##looper_clear_%d", props.index);
    ...
    if (!props.effect->params().empty()) {
        float& level = props.effect->params()[0].value;
        ImGui::SetCursorScreenPos(ImVec2(p0.x + 15 * zoom, display_y));
        ImGui::SetNextItemWidth(bar_w);
        char slider_id[64];
        std::snprintf(slider_id, sizeof(slider_id), "##looper_level_%d", props.index);
        if (ImGui::SliderFloat(slider_id, &level, 0.0f, 1.0f, "Loop Level: %.2f")) {
            level = clamp(level, 0.0f, 1.0f);
            if (props.engine) {
                props.engine->push_param_change(props.index, 0, level);
            }
        }

        char popup_id[128];
        std::snprintf(popup_id, sizeof(popup_id), "Popup_%s", slider_id);
        ...
        if (ImGui::IsItemHovered() && !props.effect->params()[0].tooltip.empty()) {
            ImGui::SetTooltip("%s", props.effect->params()[0].tooltip.c_str());
        }
    }
}
...
void ScreenComponent::render_multiband_compressor_display(ImDrawList* dl, ImVec2 p0, float pedal_width, float zoom, const ScreenProps& props) {
    ...
    auto& params = props.effect->params();
    if (params.size() < 18) return;

    const auto* entry = get_effect_color(props.effect->name());
    ImVec4 led_color = entry ? entry->led_color : ImVec4(1.0f, 1.0f, 1.0f, 1.0f);
    ...
    auto render_mb_knob = [&](ImDrawList* dl, ImVec2 center, int pi, float radius, const char* label_prefix) {
        ...
        if (is_active && !s_knob_was_active) {
            ...
            s_active_param_index = pi;
            s_param_value_before_drag = param.value;
        ...
        if (is_active && s_active_knob_id == label) {
            ...
            if (mdy != 0.0f) {
                ...
                if (new_val != param.value) {
                    param.value = new_val;
                    if (props.engine) {
                        props.engine->push_param_change(props.index, pi, new_val);
                    }
                }
            }
        }
        ...
        if (is_hovered && std::fabs(ImGui::GetIO().MouseWheel) > 0.0f) {
            ...
            if (new_val != old_val) {
                ...
                if (props.on_commit_param_change) {
                    props.on_commit_param_change(pi, old_val, new_val);
                }
            }
        }
        ...
        if (ImGui::BeginPopup(label)) {
            ...
            if (ImGui::SliderFloat("##edit", &slider_val, param.min_val, param.max_val, "%.2f")) {
                ...
                if (props.engine) {
                    props.engine->push_param_change(props.index, pi, slider_val);
                }
            }
            if (ImGui::IsItemActivated()) {
            ...
            if (props.gui_midi) {
                if (props.gui_midi->render_remove_mapping_item(props.effect->name(), param.name)) {
                    ImGui::CloseCurrentPopup();
                }
                if (props.gui_midi->render_learn_menu_item(props.effect->name(), param.name)) {
                    ImGui::CloseCurrentPopup();
                }
                ImGui::Spacing();
                if (props.gui_midi->render_remove_bypass_item(props.effect->name())) {
                    ImGui::CloseCurrentPopup();
                }
                if (props.gui_midi->render_learn_bypass_item(props.effect->name())) {
                    ImGui::CloseCurrentPopup();
                }
            } else {
        ...
        if (props.gui_midi && props.gui_midi->midi().is_learning() &&
            props.gui_midi->midi().learn_effect_name() == props.effect->name() &&
